import { STAGES, TOTAL_DAYS } from './types.js';
...
export function calcCurrentDayNumber(startDateISO, todayISO = todayDateISO()) {
...
export function isExperimentOver(startDateISO, todayISO = todayDateISO()) {
    const start = dateOnly(startDateISO);
    const today = dateOnly(todayISO);
...
export function getStageForDay(dayNumber) {
    for (const stage of STAGES) {
        if (dayNumber >= stage.rangeStart && dayNumber <= stage.rangeEnd)
            return stage;
    }
    return STAGES[STAGES.length - 1];
}
// День считается последним днём стадии (важно для подсказок "пора сделать AI-анализ")
export function isLastDayOfStage(dayNumber) {
    return STAGES.some((s) => s.rangeEnd === dayNumber);
}
export function isFirstDayOfStage(dayNumber) {
    return STAGES.some((s) => s.rangeStart === dayNumber);
}
export function addDaysISO(startISO, offsetDays) {
