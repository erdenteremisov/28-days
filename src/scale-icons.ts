...
export interface ScaleLevel {
  value: number;
  label: string;
  icon: string; // inline SVG (без внешнего <svg> тега — оборачивается снаружи)
}
...
function faceIcon(curvature: number, browTilt: number): string {
...
export const SATISFACTION_LEVELS: ScaleLevel[] = [
...
export const ENERGY_LEVELS: ScaleLevel[] = [
...
function batteryIcon(filled: number): string {
...
export const SLEEP_QUALITY_LEVELS: ScaleLevel[] = [
...
function tensionIcon(amplitude: number): string {
  if (amplitude === 0) {
...
export const TENSION_LEVELS: ScaleLevel[] = [
